/*
 * This file is generated by jOOQ.
 */
package com.pip.chatbot.jooq.weather.tables.records;


import com.pip.chatbot.jooq.weather.tables.Forecast;

import java.time.LocalDateTime;

import org.jooq.Field;
import org.jooq.Record1;
import org.jooq.Record12;
import org.jooq.Row12;
import org.jooq.impl.UpdatableRecordImpl;


/**
 * This class is generated by jOOQ.
 */
@SuppressWarnings({ "all", "unchecked", "rawtypes" })
public class ForecastRecord extends UpdatableRecordImpl<ForecastRecord> implements Record12<Integer, LocalDateTime, LocalDateTime, Float, Float, Float, Float, Float, String, String, String, String> {

    private static final long serialVersionUID = 1128409990;

    /**
     * Setter for <code>weather.forecast.id</code>.
     */
    public void setId(Integer value) {
        set(0, value);
    }

    /**
     * Getter for <code>weather.forecast.id</code>.
     */
    public Integer getId() {
        return (Integer) get(0);
    }

    /**
     * Setter for <code>weather.forecast.created_on</code>.
     */
    public void setCreatedOn(LocalDateTime value) {
        set(1, value);
    }

    /**
     * Getter for <code>weather.forecast.created_on</code>.
     */
    public LocalDateTime getCreatedOn() {
        return (LocalDateTime) get(1);
    }

    /**
     * Setter for <code>weather.forecast.date</code>.
     */
    public void setDate(LocalDateTime value) {
        set(2, value);
    }

    /**
     * Getter for <code>weather.forecast.date</code>.
     */
    public LocalDateTime getDate() {
        return (LocalDateTime) get(2);
    }

    /**
     * Setter for <code>weather.forecast.temperature</code>.
     */
    public void setTemperature(Float value) {
        set(3, value);
    }

    /**
     * Getter for <code>weather.forecast.temperature</code>.
     */
    public Float getTemperature() {
        return (Float) get(3);
    }

    /**
     * Setter for <code>weather.forecast.perceived_temperature</code>.
     */
    public void setPerceivedTemperature(Float value) {
        set(4, value);
    }

    /**
     * Getter for <code>weather.forecast.perceived_temperature</code>.
     */
    public Float getPerceivedTemperature() {
        return (Float) get(4);
    }

    /**
     * Setter for <code>weather.forecast.wind_speed</code>.
     */
    public void setWindSpeed(Float value) {
        set(5, value);
    }

    /**
     * Getter for <code>weather.forecast.wind_speed</code>.
     */
    public Float getWindSpeed() {
        return (Float) get(5);
    }

    /**
     * Setter for <code>weather.forecast.pressure</code>.
     */
    public void setPressure(Float value) {
        set(6, value);
    }

    /**
     * Getter for <code>weather.forecast.pressure</code>.
     */
    public Float getPressure() {
        return (Float) get(6);
    }

    /**
     * Setter for <code>weather.forecast.humidity</code>.
     */
    public void setHumidity(Float value) {
        set(7, value);
    }

    /**
     * Getter for <code>weather.forecast.humidity</code>.
     */
    public Float getHumidity() {
        return (Float) get(7);
    }

    /**
     * Setter for <code>weather.forecast.summary</code>.
     */
    public void setSummary(String value) {
        set(8, value);
    }

    /**
     * Getter for <code>weather.forecast.summary</code>.
     */
    public String getSummary() {
        return (String) get(8);
    }

    /**
     * Setter for <code>weather.forecast.precip_type</code>.
     */
    public void setPrecipType(String value) {
        set(9, value);
    }

    /**
     * Getter for <code>weather.forecast.precip_type</code>.
     */
    public String getPrecipType() {
        return (String) get(9);
    }

    /**
     * Setter for <code>weather.forecast.city</code>.
     */
    public void setCity(String value) {
        set(10, value);
    }

    /**
     * Getter for <code>weather.forecast.city</code>.
     */
    public String getCity() {
        return (String) get(10);
    }

    /**
     * Setter for <code>weather.forecast.icon</code>.
     */
    public void setIcon(String value) {
        set(11, value);
    }

    /**
     * Getter for <code>weather.forecast.icon</code>.
     */
    public String getIcon() {
        return (String) get(11);
    }

    // -------------------------------------------------------------------------
    // Primary key information
    // -------------------------------------------------------------------------

    @Override
    public Record1<Integer> key() {
        return (Record1) super.key();
    }

    // -------------------------------------------------------------------------
    // Record12 type implementation
    // -------------------------------------------------------------------------

    @Override
    public Row12<Integer, LocalDateTime, LocalDateTime, Float, Float, Float, Float, Float, String, String, String, String> fieldsRow() {
        return (Row12) super.fieldsRow();
    }

    @Override
    public Row12<Integer, LocalDateTime, LocalDateTime, Float, Float, Float, Float, Float, String, String, String, String> valuesRow() {
        return (Row12) super.valuesRow();
    }

    @Override
    public Field<Integer> field1() {
        return Forecast.FORECAST.ID;
    }

    @Override
    public Field<LocalDateTime> field2() {
        return Forecast.FORECAST.CREATED_ON;
    }

    @Override
    public Field<LocalDateTime> field3() {
        return Forecast.FORECAST.DATE;
    }

    @Override
    public Field<Float> field4() {
        return Forecast.FORECAST.TEMPERATURE;
    }

    @Override
    public Field<Float> field5() {
        return Forecast.FORECAST.PERCEIVED_TEMPERATURE;
    }

    @Override
    public Field<Float> field6() {
        return Forecast.FORECAST.WIND_SPEED;
    }

    @Override
    public Field<Float> field7() {
        return Forecast.FORECAST.PRESSURE;
    }

    @Override
    public Field<Float> field8() {
        return Forecast.FORECAST.HUMIDITY;
    }

    @Override
    public Field<String> field9() {
        return Forecast.FORECAST.SUMMARY;
    }

    @Override
    public Field<String> field10() {
        return Forecast.FORECAST.PRECIP_TYPE;
    }

    @Override
    public Field<String> field11() {
        return Forecast.FORECAST.CITY;
    }

    @Override
    public Field<String> field12() {
        return Forecast.FORECAST.ICON;
    }

    @Override
    public Integer component1() {
        return getId();
    }

    @Override
    public LocalDateTime component2() {
        return getCreatedOn();
    }

    @Override
    public LocalDateTime component3() {
        return getDate();
    }

    @Override
    public Float component4() {
        return getTemperature();
    }

    @Override
    public Float component5() {
        return getPerceivedTemperature();
    }

    @Override
    public Float component6() {
        return getWindSpeed();
    }

    @Override
    public Float component7() {
        return getPressure();
    }

    @Override
    public Float component8() {
        return getHumidity();
    }

    @Override
    public String component9() {
        return getSummary();
    }

    @Override
    public String component10() {
        return getPrecipType();
    }

    @Override
    public String component11() {
        return getCity();
    }

    @Override
    public String component12() {
        return getIcon();
    }

    @Override
    public Integer value1() {
        return getId();
    }

    @Override
    public LocalDateTime value2() {
        return getCreatedOn();
    }

    @Override
    public LocalDateTime value3() {
        return getDate();
    }

    @Override
    public Float value4() {
        return getTemperature();
    }

    @Override
    public Float value5() {
        return getPerceivedTemperature();
    }

    @Override
    public Float value6() {
        return getWindSpeed();
    }

    @Override
    public Float value7() {
        return getPressure();
    }

    @Override
    public Float value8() {
        return getHumidity();
    }

    @Override
    public String value9() {
        return getSummary();
    }

    @Override
    public String value10() {
        return getPrecipType();
    }

    @Override
    public String value11() {
        return getCity();
    }

    @Override
    public String value12() {
        return getIcon();
    }

    @Override
    public ForecastRecord value1(Integer value) {
        setId(value);
        return this;
    }

    @Override
    public ForecastRecord value2(LocalDateTime value) {
        setCreatedOn(value);
        return this;
    }

    @Override
    public ForecastRecord value3(LocalDateTime value) {
        setDate(value);
        return this;
    }

    @Override
    public ForecastRecord value4(Float value) {
        setTemperature(value);
        return this;
    }

    @Override
    public ForecastRecord value5(Float value) {
        setPerceivedTemperature(value);
        return this;
    }

    @Override
    public ForecastRecord value6(Float value) {
        setWindSpeed(value);
        return this;
    }

    @Override
    public ForecastRecord value7(Float value) {
        setPressure(value);
        return this;
    }

    @Override
    public ForecastRecord value8(Float value) {
        setHumidity(value);
        return this;
    }

    @Override
    public ForecastRecord value9(String value) {
        setSummary(value);
        return this;
    }

    @Override
    public ForecastRecord value10(String value) {
        setPrecipType(value);
        return this;
    }

    @Override
    public ForecastRecord value11(String value) {
        setCity(value);
        return this;
    }

    @Override
    public ForecastRecord value12(String value) {
        setIcon(value);
        return this;
    }

    @Override
    public ForecastRecord values(Integer value1, LocalDateTime value2, LocalDateTime value3, Float value4, Float value5, Float value6, Float value7, Float value8, String value9, String value10, String value11, String value12) {
        value1(value1);
        value2(value2);
        value3(value3);
        value4(value4);
        value5(value5);
        value6(value6);
        value7(value7);
        value8(value8);
        value9(value9);
        value10(value10);
        value11(value11);
        value12(value12);
        return this;
    }

    // -------------------------------------------------------------------------
    // Constructors
    // -------------------------------------------------------------------------

    /**
     * Create a detached ForecastRecord
     */
    public ForecastRecord() {
        super(Forecast.FORECAST);
    }

    /**
     * Create a detached, initialised ForecastRecord
     */
    public ForecastRecord(Integer id, LocalDateTime createdOn, LocalDateTime date, Float temperature, Float perceivedTemperature, Float windSpeed, Float pressure, Float humidity, String summary, String precipType, String city, String icon) {
        super(Forecast.FORECAST);

        set(0, id);
        set(1, createdOn);
        set(2, date);
        set(3, temperature);
        set(4, perceivedTemperature);
        set(5, windSpeed);
        set(6, pressure);
        set(7, humidity);
        set(8, summary);
        set(9, precipType);
        set(10, city);
        set(11, icon);
    }
}
